...
### 6. Conteggi Statistiche Girlfriend
...
Future<Map<String, dynamic>> _loadGirlfriendStats(String girlfriendId) async {
  ...
  return {
    ...
    'isFavorite': isFavorite.data?['favorite_girlfriend_id'] == girlfriendId
  };
}
```
...
**File**: `lib/screens/girlfriend_gallery_screen.dart:196,205`
...
### 8. Eliminazione Media Girlfriend
**File**: `backend/services/supabase-storage.js:215`

**Problema**: Quando si elimina girlfriend, i media rimangono in storage

**Azione**:
```javascript
async function deleteGirlfriendFiles(girlfriendId) {
  try {
    // Elimina avatar
    await supabase.storage
      .from('avatars')
      .remove([`girlfriends/${girlfriendId}`]);
    
    // Elimina chat images
    const { data: images } = await supabase.storage
      .from('chat-images')
      .list('', { prefix: `${girlfriendId}/` });
    
    if (images && images.length > 0) {
      const imagePaths = images.map(img => `${girlfriendId}/${img.name}`);
    ...
    if (videos && videos.length > 0) {
      const videoPaths = videos.map(v => `${girlfriendId}/${v.name}`);
    ...
    if (audios && audios.length > 0) {
      const audioPaths = audios.map(a => `${girlfriendId}/${a.name}`);
      await supabase.storage.from('chat-audios').remove(audioPaths);
    }
    
    console.log(`✅ Deleted all files for girlfriend ${girlfriendId}`);
  } catch (error) {
    console.error('Error deleting girlfriend files:', error);
    throw error;
  }
}
```
...
class Analytics {
  ...
  async trackMessageSent(userId, girlfriendId, type) {
...
- [ ] Task 8: Eliminazione media girlfriend

### Sprint 3 (Settimana 5-6)
- [ ] Task 5: Notifiche push
- [ ] Task 6: Statistiche girlfriend
