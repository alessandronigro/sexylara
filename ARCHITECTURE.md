...
**ThrillMe** è una piattaforma di chat AI multi-girlfriend con supporto per:
- Chat 1-to-1 con AI girlfriends personalizzate
...
│   ├── girlfriend.js     # CRUD girlfriends
...
Client → Send Message { text, traceId, girlfriend_id?, group_id? }
...
│   ├── girlfriend_service.dart   # API girlfriends
...
/chat                      → ContactsScreen (lista girlfriends)
/chat/:id                  → ChatScreen (1-to-1)
/chat/profile/:id          → GirlfriendProfileScreen
/chat/gallery/:id          → GirlfriendGalleryScreen
/create-girlfriend         → CreateGirlfriendScreen
...
id              UUID PRIMARY KEY
user_id         UUID REFERENCES auth.users(id)
girlfriend_id   UUID REFERENCES girlfriends(id)
session_id      TEXT
...
member_id       UUID -- può essere user_id o girlfriend_id
member_type     TEXT (user/ai)
girlfriend_id   UUID REFERENCES girlfriends(id) -- per compatibilità
...
id              UUID PRIMARY KEY
group_id        UUID REFERENCES groups(id) ON DELETE CASCADE
sender_id       UUID -- user_id o girlfriend_id
type            TEXT (text/image/video/audio)
content         TEXT
created_at      TIMESTAMP
```
...
receiver_id     UUID -- user_id o girlfriend_id
...
#### **avatars**
- Avatar utenti e girlfriends
- Path: `{userId}/{filename}`
- Pubblico

#### **chat-images**
- Immagini generate in chat
- Path: `{userId}/{girlfriendId}/{filename}`
- Pubblico
...
- **Prompt**: Costruito da caratteristiche girlfriend + contesto
...
- **Linea 215**: Implementare eliminazione chat media quando si elimina girlfriend
