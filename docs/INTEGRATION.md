...
|----------|---------|------------------|--------|
| `GET /api/chat-history/:userId/:npcId` | ✅ server-api.js | ✅ chat_service.dart | chat_screen.dart |
| `GET /api/npc-gallery/:userId/:npcId` | ✅ server-api.js | ✅ npc_service.dart | npc_gallery_screen.dart |
| `POST /api/generate-avatar` | ✅ server-api.js | ✅ npc_service.dart | create_npc_screen.dart |
...
| `DELETE /api/npc/:id` | ✅ server-api.js | ✅ Implementato | ✅ OK |
...
|---------|------------------|------------------|----------|
| **Statistiche NPC** | `GET /api/npcs/:id/stats` | npc_service.dart | 🟡 Media |
| **Sistema Preferiti** | `POST /api/npcs/:id/favorite` | npc_service.dart | 🟡 Media |
| **Messaggi Random Persistiti** | `POST /api/messages/random` | random_message_service.dart | 🟡 Media |
| **Download Immagini** | - (client-side) | npc_gallery_screen.dart | 🟡 Media |
| **Share Immagini** | - (client-side) | npc_gallery_screen.dart | 🟡 Media |
...
| `media_generation_started` | Server → Client | ✅ server-ws.js | ✅ chat_service.dart → chat_screen.dart | chat_screen.dart |
| `media_generation_completed` | Server → Client | ✅ server-ws.js | ✅ chat_service.dart → chat_screen.dart | chat_screen.dart |
| `media_generation_failed` | Server → Client | ✅ server-ws.js | ✅ chat_service.dart → chat_screen.dart | chat_screen.dart |
...
| `media_generation_*` | Server → Client | Placeholder media realtime | ✅ Implementato |

---

## 📋 Task di Consolidamento - Priorità Integrate

### ✅ Nuove integrazioni completate
- Placeholder media in stile WhatsApp: il frontend inserisce `PendingMedia` via WS (`media_generation_*`) e sostituisce con il media finale o stato failed senza bloccare l'input.
- Router LLM (Venice/Ollama) lato backend seleziona modello in base a `npc.preferences.tone_mode`, con fallback safe.
...
7. **Endpoint Statistiche NPC**
   - Nuovo: `GET /api/npcs/:id/stats`
...
8. **Endpoint Sistema Preferiti**
   - Nuovo: `POST /api/npcs/:id/favorite`
   - Azione: Toggle npc preferita
   - Impatto: Feature preferiti funzionante
   - Tempo: 1-2 ore
   - **Richiede**: Implementazione frontend

9. **Eliminazione Media NPC** (TODO.md #8)
   - File: `backend/services/supabase-storage.js:215`
   - Azione: Eliminare media quando si elimina npc
...
10. **Implementare Download Immagini** (FLUTTER_ANALYSIS.md #2)
    - File: `lib/screens/npc_gallery_screen.dart:196`
...
11. **Implementare Share Immagini** (FLUTTER_ANALYSIS.md #3)
    - File: `lib/screens/npc_gallery_screen.dart:205`
    - Azione: Implementare share con `share_plus`
    - Impatto: Feature completa
    - Tempo: 1-2 ore

12. **Implementare Statistiche NPC** (FLUTTER_ANALYSIS.md #4-6)
...
14. **State Management Riverpod** (FLUTTER_ANALYSIS.md - Miglioramento #1)
    - Azione: Creare provider per npcs, groups, messages
...
- [ ] Task #7: Endpoint statistiche npc (2-3h)
- [ ] Task #8: Endpoint sistema preferiti (1-2h)
- [ ] Task #9: Eliminazione media npc (2-3h)

**Frontend**:
- [ ] Task #10: Download immagini (2-3h)
- [ ] Task #11: Share immagini (1-2h)
- [ ] Task #12: Statistiche npc (3-4h)
