...
│   ├── contacts_screen.dart           # Lista girlfriends
│   ├── chat_screen.dart               # Chat 1-to-1
│   ├── create_girlfriend_screen.dart
│   ├── girlfriend_profile_screen.dart
│   ├── girlfriend_gallery_screen.dart
...
│   ├── girlfriend_service.dart        # CRUD girlfriends
...
**Soluzione**:
```dart
Future<void> _sendRandomMessage(String girlfriendId, String message) async {
...
#### 2. **girlfriend_gallery_screen.dart:196**
```dart
...
#### 3. **girlfriend_gallery_screen.dart:205**
```dart
...
#### 4-6. **girlfriend_profile_screen.dart:286-288**
```dart
...
Future<Map<String, dynamic>> _loadGirlfriendStats(String girlfriendId) async {
  final userId = SupabaseService.currentUser?.id;
  ...
  return {
    ...
    'isFavorite': favoriteResp?['favorite_girlfriend_id'] == girlfriendId
  };
}

// Usa in build
FutureBuilder<Map<String, dynamic>>(
  future: _loadGirlfriendStats(widget.girlfriendId),
...
- [ ] Implementare statistiche girlfriend
...
```dart
// lib/providers/girlfriends_provider.dart
final girlfriendsProvider = FutureProvider<List<Girlfriend>>((ref) async {
  return GirlfriendService().getGirlfriends();
});
...
final messagesProvider = StreamProvider.family<List<Message>, String>((ref, girlfriendId) {
  return ChatService().messages.where((msg) => msg.girlfriendId == girlfriendId);
});
```
...
class LocalDatabase {
  ...
  Future<List<Message>> getCachedMessages(String girlfriendId) async {
    final db = await database;
    final maps = await db.query(
      'messages',
      where: 'girlfriend_id = ?',
      whereArgs: [girlfriendId],
...
- GET  /api/girlfriends/:id/stats (per statistiche)
- POST /api/girlfriends/:id/favorite (per preferiti)
- GET  /api/messages/random/:girlfriendId (per messaggi random)
```
...
✅ Implementati:
- connect (user_id)
- message (text, traceId, girlfriend_id)
